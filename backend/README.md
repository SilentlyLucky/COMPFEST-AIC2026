...
- `ai/listing/`: Qwen2.5-VL 3B LoRA adapter and lazy inference code for title
  and description. The base model `Qwen/Qwen2.5-VL-3B-Instruct` is downloaded
  or read from the Hugging Face cache when the worker first loads the model.
- `ai/pricing/`: lazy TF-IDF catalog retrieval for category routing and market
  price evidence.
...
prompt. The catalog vote and price interval currently have no calibrated score,
so their confidence remains `null` until calibration artifacts are published.
...
synthetic demo catalog from `model_harga`.
...
`/health/live` only shows that the process is alive. `/health/ready` returns
HTTP 503 until the CUDA model runtime and versioned catalog can start.
